...
| 当前已验证代码 head | `dd822e85`（`dev`；本机 Rust 后端三件套全绿 1751 passed / 2 ignored、前端 CI 全绿、完整 CI 与 macOS Ad Hoc 构建均通过）。本轮 `sync-2026-09-18`（26 个提交）**已验证**，见「同步审计日志」2026-09-18 条目 |
...
最终 **26 个提交（23 代码 + 3 文档），32 个文件，+2470 / −127**。

其中 4 个**代码修复**提交（`5e0bd59a` 排版、`e60e488b` 常量、`8e2605e7` 测试锁、
`dd822e85` 夹具与 fail-closed 守卫）是本机预跑 CI 时补的，**没有一个是上游
提交的新搬运项**，全部是前 19 个提交自身的问题。另有 3 个**文档**提交
（`ab899b1c` 本轮台账、`f5d4ecec` 验证与 CI/构建结果、`71a6dee2` v19 干跑）。
详见下方「验证」段与「验证手段」的 2026-09-18 补记。
