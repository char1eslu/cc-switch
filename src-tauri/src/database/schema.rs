...
impl Database {
    ...
    pub(crate) fn create_tables_on_conn(conn: &Connection) -> Result<(), AppError> {
        ...
        //
        // 兼容旧数据库：
        // - 老版本 proxy_config 是单例表（没有 app_type 列），此时不能执行双行 seed insert；
        // - 旧表会在 apply_schema_migrations() 中迁移为双行结构后再插入。
        ...
        // 兼容：若旧版 proxy_config 仍为单例结构（无 app_type），则在启动时直接转换为双行结构
    ...
    pub(crate) fn apply_schema_migrations_on_conn(conn: &Connection) -> Result<(), AppError> {
        ...
        let is_legacy_fork_schema = Self::table_exists(conn, "settings")?
            ...
            && ((version == 17 && has_desktop_mcp_column && has_legacy_two_app_proxy_constraint)
    ...
    fn seed_model_pricing(conn: &Connection) -> Result<(), AppError> {
        let pricing_data = [
            ...
            // 官方价页（api-docs.deepseek.com/quick_start/pricing）直接挂 USD，不再需要
            // CNY 折算。高峰时段 = 北京时间 9:00-12:00 与 14:00-18:00，共 7h/天；
            // 其余 17h 为空闲档。
            //
            // 🔴 本表每模型仅一行、无时段维度，**统一录高峰档**：
    ...
    fn repair_current_model_pricing(conn: &Connection) -> Result<(), AppError> {
        let pricing_fixes = [
            ...
            // 2026-07 那次「chat/reasoner 降为 V4 Flash 别名」的收敛条目，
            // 所以这两个模型是单跳到位，不是两跳。照抄上游守卫值会永不命中。
            (
                "deepseek-chat",
                "DeepSeek Chat",
                "0.44",
            ...
            // M2.5 必须先经旧条目的 0.12 -> 0.15，再匹配本轮守卫。
            (
                "minimax-m2",
                "MiniMax M2",
                "0.30",
