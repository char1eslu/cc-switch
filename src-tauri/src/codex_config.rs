...
/// `experimental_bearer_token` short-circuits resolves to whatever `auth.json`
...
/// login to a stale base_url. The renamed table is also normalized into a
...
///
/// Only tables that short-circuits request auth (`env_key` or an
...
pub fn prepare_codex_provider_live_config(
    ...
    // The legacy reroute shape (built-in `openai` provider + top-level
    // `openai_base_url`) has no provider table to carry the key — rewrite it
    // into a cc-switch-owned custom table before injection.
