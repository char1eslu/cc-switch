...
struct ChatToResponsesState {
    ...
    /// 下一个允许发出 output_item.added 的 Chat tool 索引。
    ///
    /// 并行 tool call 的 id/name 可能在后续 delta 才补齐，谁先补齐就先发会
    /// 打乱顺序；按 Chat index 连续放行可保证客户端看到的顺序与上游一致。
...
impl ChatToResponsesState {
    ...
    fn push_tool_call_delta(&mut self, tool_call: &Value, reasoning: Option<&str>) -> Vec<Bytes> {
...

        {
            ...
            // 续传 delta 常把 id/name 回填为空串（实测 dashscope）。空串不能覆盖
            // 已收到的值，否则整个 tool call 的身份丢失，下游看到 call_id=""。
    ...
    /// 按 Chat index 顺序释放已就绪的并行 tool call。
    ///
    /// 身份字段（id/name）可能比 index 更晚到达：若谁先凑齐就先发，输出顺序
    /// 会与客户端按 index 的预期不一致，并行调用被张冠李戴。这里只从
    /// `next_tool_index_to_add` 起连续释放，遇到未就绪的就停，保证顺序稳定。
...
mod tests {
    ...
    #[tokio::test]
    async fn upstream_preserves_tool_identity_across_empty_continuation_deltas() {
        let output = collect(vec![
    ...
    #[tokio::test]
    async fn upstream_preserves_parallel_tool_order_when_earlier_name_arrives_late() {
        let output = collect(vec![
    ...
    #[tokio::test]
    async fn upstream_finalization_keeps_valid_call_after_unnamed_earlier_call() {
        let output = collect(vec![
    ...
    #[tokio::test]
    async fn upstream_finalization_keeps_non_contiguous_tool_index() {
        let output = collect(vec![
