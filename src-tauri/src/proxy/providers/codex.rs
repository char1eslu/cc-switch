...
pub fn codex_provider_uses_anthropic(provider: &Provider) -> bool {
    ...
    codex_settings_uses_anthropic(&provider.settings_config)
}

/// Same Anthropic detection as [`codex_provider_uses_anthropic`] but operating on
/// a raw settings `Value` (no `meta`/`Provider` available). Used by catalog
...
pub(crate) fn codex_settings_uses_anthropic(settings: &JsonValue) -> bool {
    if let Some(api_format) = settings
        .get("api_format")
        .and_then(|v| v.as_str())
        .or_else(|| settings.get("apiFormat").and_then(|v| v.as_str()))
    {
        return is_anthropic_wire_api(api_format);
    }
