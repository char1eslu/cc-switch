//! Moonshot/Kimi rejects tool schemas with sibling keywords beside `$ref`.
//! Move the reference into `allOf` only for its Chat endpoint; preserve literal
//! data and other providers' schemas to avoid changing prompt-cache prefixes.
