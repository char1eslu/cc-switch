...
fn build_tool_action_line(
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        //    Windows batch 语义不适合跨 wsl.exe;这里统一替换为 POSIX 版安装/更新命令。
        if let Some(distro) = wsl_distro_for_tool(tool) {
...
///
/// 为何不通用覆盖所有工具:各家预发布 tag 命名互不统一；先只为 Claude Code
/// 启用,避免把不稳定通道误报给稳定用户。
...
///
/// 该端点的响应体就是 dist-tags 对象本身（几十到几千字节）；而 `/{package}`
/// 返回的是含每个历史版本元数据的完整 packument，claude-code / codex 这类高频
/// 发版的包已有十几到二十几 MB，一次刷新要下几十 MB。scoped 包名的 `/` 按
/// registry 约定转义成 `%2f`。
...
/// 版本探测请求的请求级超时。全局客户端是给代理转发用的（总超时 600s /
/// 连接 30s），探测 latest 若沿用它，registry 被阻断或握手后挂起时卡片会一直
/// 「加载中」；探测拿不到就显示「未知」即可。
///
/// 取自上游 `b1250dc7`（该提交其余内容面向 Hermes，不在本 fork 范围内）。
const LATEST_PROBE_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(15);

/// 拉取 npm 包的完整 dist-tags（单次请求即含 latest/next/beta/...）。
///
/// 与 GitHub / PyPI 两条来源一样套 `LATEST_PROBE_TIMEOUT`：取不到就返回 None，
/// 由调用方显示「未知」，而不是沿用全局客户端的 600s 总超时让卡片一直
/// 「加载中」。包不存在时端点返回 404 与一个 JSON 字符串体，解析成 Map 失败，
/// 同样落到 None。
...
/// ② Codex 独立安装器（`CODEX_INSTALL_DIR`）→ 带 `CODEX_INSTALL_DIR` / `CODEX_HOME`
///    重跑官方 installer;`codex update` 断网时假成功(见 `CODEX_INSTALL_UNIX`)。
...
///   Anthropic 已将 native installer 列为首推、把 npm 列为传统方式。
...
/// - Windows 原生不启用:claude.ai/install.sh 是 bash 脚本,
...
fn extract_env_vars_from_config(
    ...
    // 处理 env 字段（Claude / Claude Desktop）
...
fn launch_terminal_with_env(
    ...
    let cleanup_path = shell_single_quote(&config_file.to_string_lossy());
    let command = format!("trap 'rm -f {cleanup_path}' EXIT; {command}");
    let terminal =
        crate::settings::get_preferred_terminal().unwrap_or_else(|| "terminal".to_string());

    let result = crate::session_manager::terminal::launch_command(
        &terminal,
        &command,
