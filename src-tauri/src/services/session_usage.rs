...
/// 一轮扫描开头一次性预取的 `session_log_sync` 游标。
///
/// 替代逐文件的单行查询：文件数随历史只增不减，逐文件查询意味着每轮上千次
/// 取锁。
...
mod tests {
    ...
    /// 测试夹具：把游标的 mtime 压到 0，让下一轮必然通过 mtime 门，
    /// 从而确定性地只测字节游标行为（不依赖文件系统时间戳粒度）。
    fn reset_cursor_mtime(db: &Database, file_path: &Path) -> Result<(), AppError> {
        let conn = lock_conn!(db.conn);
        conn.execute(
    ...
    fn cursor_of(db: &Database, file_path: &Path) -> SyncCursor {
    ...
    fn assistant_line(msg_id: &str, output: u32) -> String {
        format!(
            r#"{{"type":"assistant","message":{{"id":"{msg_id}","model":"claude-opus-4-8","usage":{{"input_tokens":5,"output_tokens":{output},"cache_read_input_tokens":0,"cache_creation_input_tokens":0}}}},"timestamp":"2026-08-27T13:01:23Z","sessionId":"session-x"}}"#
        )
    }

    fn logged_request_ids(db: &Database) -> Result<Vec<String>, AppError> {
        let conn = lock_conn!(db.conn);
        let mut stmt = conn
    ...
    #[test]
    fn byte_cursor_only_reads_the_appended_tail() -> Result<(), AppError> {
        // 追加路径：第二轮只读新增字节，已导入的行不重复入库，游标推进到 EOF。
        let db = Database::memory()?;
        let tmp = std::env::temp_dir().join(format!("cc-switch-test-{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&tmp).unwrap();
        let file = tmp.join("append.jsonl");
        fs::write(&file, format!("{}\n", assistant_line("msg_a", 10))).unwrap();

        let first = sync_single_file(&db, &file, None)?;
        assert_eq!(first.imported, 1);
        let cursor = cursor_of(&db, &file);
        assert_eq!(
            cursor.last_byte_offset,
            Some(fs::metadata(&file).unwrap().len() as i64),
            "游标必须推进到最后一个完整行之后"
        );
        assert!(cursor.last_tail_fingerprint.is_some(), "必须写入尾部指纹");

        {
            use std::io::Write;
            let mut handle = fs::OpenOptions::new().append(true).open(&file).unwrap();
            writeln!(handle, "{}", assistant_line("msg_b", 20)).unwrap();
        }
        reset_cursor_mtime(&db, &file)?;

        let second = sync_single_file(&db, &file, Some(&cursor_of(&db, &file)))?;
        assert_eq!(second.imported, 1, "只应导入新追加的那一条");
        assert_eq!(second.skipped, 0, "已提交区间不应被重读");
        assert_eq!(
            logged_request_ids(&db)?,
            vec!["session:msg_a".to_string(), "session:msg_b".to_string()]
        );
    ...
    #[test]
    fn incomplete_tail_is_imported_but_not_committed() -> Result<(), AppError> {
        // 半行（无换行）可解析时照常导入，但游标不越过它；补全后不会因
        // "行已数过"而被永久跳过（旧行号游标的 bug）。
        let db = Database::memory()?;
        let tmp = std::env::temp_dir().join(format!("cc-switch-test-{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&tmp).unwrap();
        let file = tmp.join("partial.jsonl");
        let complete = format!("{}\n", assistant_line("msg_done", 10));
        let half = assistant_line("msg_half", 20);
        fs::write(&file, format!("{complete}{half}")).unwrap();

        let first = sync_single_file(&db, &file, None)?;
        assert!(first.incomplete_tail, "无换行尾段必须标记为未完成");
        assert_eq!(first.imported, 2, "可解析的半行仍然导入");
        assert_eq!(
            cursor_of(&db, &file).last_byte_offset,
            Some(complete.len() as i64),
            "游标只能停在最后一个完整行之后"
        );

        // 补上换行 + 再追加一行：半行区间被重读，request_id 去重挡住重复入库
        fs::write(
            &file,
            format!("{complete}{half}\n{}\n", assistant_line("msg_next", 30)),
        )
        .unwrap();
        reset_cursor_mtime(&db, &file)?;
        ...
        assert_eq!(second.imported, 1, "只有真正的新行入库");
        assert_eq!(second.skipped, 1, "重读的半行被 request_id 去重挡下");
        assert_eq!(
            logged_request_ids(&db)?,
            vec![
                "session:msg_done".to_string(),
                "session:msg_half".to_string(),
                "session:msg_next".to_string()
            ]
        );
    ...
    #[test]
    fn external_rewrite_pins_cursor_without_replaying() -> Result<(), AppError> {
        // 同尺寸重写（size 检测不出来）必须靠尾部指纹发现：游标钉到 EOF、
        // 旧区间一概不重放，避免已剪明细双算。
        let db = Database::memory()?;
        let tmp = std::env::temp_dir().join(format!("cc-switch-test-{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&tmp).unwrap();
        let file = tmp.join("rewritten.jsonl");
        fs::write(&file, format!("{}\n", assistant_line("msg_orig", 10))).unwrap();

        sync_single_file(&db, &file, None)?;
        let size_before = fs::metadata(&file).unwrap().len();

        // 同长度不同内容（message_id 位宽相同）
        fs::write(&file, format!("{}\n", assistant_line("msg_ORIG", 10))).unwrap();
        assert_eq!(
        ...
        let after = sync_single_file(&db, &file, Some(&cursor_of(&db, &file)))?;
        assert_eq!(after.pinned_rewrite, Some("重写"));
        assert_eq!(after.imported, 0, "重写区间不得重放");
        assert_eq!(
            cursor_of(&db, &file).last_byte_offset,
            Some(size_before as i64),
            "游标钉到当前 EOF"
        );
        assert_eq!(
            logged_request_ids(&db)?,
            vec!["session:msg_orig".to_string()],
            "重写后的内容不入库，旧记录保持不变"
        );
    ...
    #[test]
    fn external_truncation_pins_cursor_to_eof() -> Result<(), AppError> {
        let db = Database::memory()?;
        let tmp = std::env::temp_dir().join(format!("cc-switch-test-{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&tmp).unwrap();
        let file = tmp.join("truncated.jsonl");
        fs::write(
            &file,
            format!(
                "{}\n{}\n",
                assistant_line("msg_1", 10),
                assistant_line("msg_2", 20)
            ),
        )
        .unwrap();
        ...
        let after = sync_single_file(&db, &file, Some(&cursor_of(&db, &file)))?;
        assert_eq!(after.pinned_rewrite, Some("截断"));
        assert_eq!(cursor_of(&db, &file).last_byte_offset, Some(0));
    ...
    #[test]
    fn legacy_line_cursor_is_converted_byte_wise() -> Result<(), AppError> {
        // 升级路径：存量行只有 last_line_offset、字节游标为 NULL。必须按行
        // 跳过前 L 行后继续增量，不能全量重读（会把已剪明细双算）。
        let db = Database::memory()?;
        let tmp = std::env::temp_dir().join(format!("cc-switch-test-{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&tmp).unwrap();
        let file = tmp.join("legacy.jsonl");
        let first_line = format!("{}\n", assistant_line("msg_old", 10));
        fs::write(
            &file,
            format!("{first_line}{}\n", assistant_line("msg_new", 20)),
        )
        .unwrap();
        ...
        let result = sync_single_file(&db, &file, Some(&legacy_cursor))?;
        assert_eq!(result.imported, 1, "只导入旧游标之后的行");
        assert_eq!(
            logged_request_ids(&db)?,
            vec!["session:msg_new".to_string()],
            "旧游标已覆盖的行不得重导"
        );
        let cursor = cursor_of(&db, &file);
        assert_eq!(
        ...
        assert_eq!(cursor.last_line_offset, 0, "转换后行号游标置 0");
