...
impl ProxyService {
    ...
    ///
    /// 代理服务器的路由已经根据 API 端点自动区分 Claude 与 Codex。
...
mod tests {
    ...
    async fn seed_distinct_app_proxy_configs(db: &Database) -> Vec<Value> {
        ...
        // fork 只保留 Claude / Codex 两个应用，gemini 与 grokbuild 行已裁剪。
        for (app, retries) in [("claude", 6), ("codex", 0)] {
...
"#
    ...
    #[tokio::test]
    #[serial]
    async fn codex_takeover_preserve_disabled_writes_config_only() {
        let _home = TempHome::new();
        crate::settings::reload_settings().expect("reload settings");
        crate::settings::update_settings(crate::settings::AppSettings {
...
"#
        ...
        // Since Codex 0.149 third-party writes are config-only on every path:
        // preservation off deletes auth.json (the OAuth login is gone either
        // way — it used to be overwritten by the placeholder) and the takeover
        // placeholder rides in config.toml as the provider-scoped bearer token,
        // which is what Codex actually sends to the local proxy.
        assert!(
        ...
        assert!(
            ...
            "the takeover placeholder must ride in config.toml; got:\n{live_config}"
        );
...
"#
        ...
        // Config-only writes (Codex >= 0.149): the takeover placeholder rides
        // in the active provider table instead of auth.json.
        assert_eq!(
            parsed_live
                .get("model_providers")
                .and_then(|v| v.get("deepseek"))
...
"#
    ...
    async fn bulk_backup_skips_all_when_live_is_proxy_placeholder() {
        ...
        // Seed good backups for both proxy-managed apps
        let good_backup = serde_json::to_string(&json!({
            "env": {
                "ANTHROPIC_AUTH_TOKEN": "real-token"
            }
        ...
        // Seed both Live files with proxy placeholders
        service
            .write_claude_live(&json!({
                "env": {
                    "ANTHROPIC_AUTH_TOKEN": PROXY_TOKEN_PLACEHOLDER,
...
"#,
        ...
        // Call bulk backup: must skip both apps
        service
            .backup_live_configs()
            .await
            .expect("bulk backup should succeed (no-op when all live are placeholders)");

        // Both good backups must still be intact
